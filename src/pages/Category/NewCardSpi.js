...
function NewCardSpi() {
  ...
  const handleFilter = async (value) => {
    return await axios
      .get(`http://localhost:3001/users?subcat=${value}&cat=بهاره`)
      .then((response) => {
  ...
  const handleRefresh = async () => {
    return await axios
      .get(`http://localhost:3001/users?cat=بهاره`)
      .then((response) => {
        setAllFruits(response.data);
      });
  };
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/users?cat=بهاره").then((response) => {
      setAllFruits(response.data);
    });
  }, []);
