...
function NewCardWi() {
  ...
  const handleFilter = async (value) => {
    return await axios
      .get(`http://localhost:3001/users?subcat=${value}&cat=زمستانه`)
      .then((response) => {
  ...
  const handleRefresh = async () => {
    return await axios
      .get(`http://localhost:3001/users?cat=زمستانه`)
      .then((response) => {
        setAllFruits(response.data);
      });
  };
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/users?cat=زمستانه").then((response) => {
      setAllFruits(response.data);
    });
  }, []);
